...
- **Service PTY parity on macOS launchd.** `generatePlist` now wraps the invocation in `/usr/bin/script -q /dev/null <claudeBin> <args>` (BSD syntax). launchd services run without a controlling TTY by default — same shape as systemd — so the SessionEnd-hook failure mode fixed on Linux in #9 could in principle hit Mac. Validated locally that the wrapper does not break the happy path (`script -q /dev/null claude --version` exits 0 with clean output); runtime proof of the bug on Mac was not attempted as it would require installing a live launchd service. Preemptive parity using the already-validated mechanism. [#16](https://github.com/crisandrews/ClawCode/pull/16).
